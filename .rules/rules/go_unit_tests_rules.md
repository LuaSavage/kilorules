...
#### For interfaces without embedding:
+ Use the `go-mockery-descriptor` helper tool (see `go_mockery_helper_helper.md` rule)
+ This automatically generates call structures and mock factory functions
#### For interfaces with embedding:
+ Mockery is not compatible with embedded interfaces
+ Write mocks manually in a separate file
+ File naming convention: edited_mock_{InterfaceName}.go
+ Follow the same call structure pattern described below  
...
**For auto-generated mocks (no embedding):**  
Run `go generate` after adding test cases - the call structures will be generated automatically.
**For manually written mocks (with embedding):**  
...
- **Test code first:** Write the table-driven tests at the top of the file
- **Helpers at the bottom:** Place helper functions, mock factories, and call structures at the end of the file if they're necessary
- **Global matchers at the top:** Move `anyCtx`, `anyTx`, and other common variables to the top of the file if they're used across multiple tests
